...
class ImageLoader:
    ...
    def file_dialog_filter(cls) -> str:
        ...
        standard = " ".join(
            f"*{ext}" for ext in sorted(cls.STANDARD_EXTENSIONS)
        )
        raw = " ".join(
            f"*{ext}" for ext in sorted(cls.RAW_EXTENSIONS)
        )
        return (
    ...
    def load(cls, file_path: str | Path) -> ImageDocument:
        """Load an image file into a new ImageDocument."""
        path = Path(file_path)
        ...
        document.load(image=image, file_path=path)
        return document
