...
class MasksPanel(QWidget):
    """Mask controls: Density, Feather, Invert."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._invert_check = QCheckBox("Odwróc maske")
        self._invert_check.setStyleSheet("color: #AAAAAA; padding: 4px 0px;")
        self._invert_check.stateChanged.connect(self._emit_change)
        layout.addWidget(self._invert_check)

        reset_btn = QPushButton("Resetuj maske")
    ...
    def get_values(self) -> dict[str, float]:
        values = {k: float(v.value) for k, v in self._sliders.items()}
        values["mask_inverted"] = 1.0 if self._invert_check.isChecked() else 0.0
        return values
    ...
    def set_values(self, values: dict[str, float]) -> None:
        for k, v in values.items():
            ...
            elif k == "mask_inverted":
                self._invert_check.setChecked(v > 0.5)
